...
<\body>
  ...
  <\proof>
    ...
    Let <math|\<forall\>r<rsub|1>\<in\>\<cal-X\>> given. We have a Markov
    chain generated by Metropolis algorithm starting at <math|r<rsub|1>>, say
    <math|<around*|{|r<rsub|1>,r<rsub|2>,\<ldots\>,r<rsub|N>|}>>. We want to
    prove that <math|<around*|{|r<rsub|1>,r<rsub|2>,\<ldots\>,r<rsub|N>|}>>
    approximately obeys distribution <math|p<around*|(|x|)>>. This proof has
    three steps.

    S1) Define <math|h> by <math|<around*|{|r<rsub|1>,r<rsub|2>,\<ldots\>,r<rsub|N-1>|}>\<sim\>h>,
    and <math|h<rprime|'>> by <math|<around*|{|r<rsub|2>,r<rsub|3>,\<ldots\>,r<rsub|N>|}>\<sim\>h<rprime|'>>.
    That is, the histogram of <math|<around*|{|r<rsub|1>,r<rsub|2>,\<ldots\>,r<rsub|N-1>|}>>,
    after normalization, can be fitted by <math|h>, and the same for
    <math|h<rprime|'>>. We will proof first that
    <math|h=h<rprime|'>+o<around*|(|1|)>> on <math|<with|math-font|cal|X>> as
    <math|N\<rightarrow\>+\<infty\>>. Indeed, if <math|N> is large enough,
    <math|<around*|{|r<rsub|1>,r<rsub|2>,\<ldots\>,r<rsub|N-1>|}>\<sim\>h>,
    since dropping a single element will affect little on the fitting of
    histogram (will be reviewed later in remark <reference|remark: Burn-in>).
    So, <math|h=h<rprime|'>+o<around*|(|1|)>> on
    <math|<with|math-font|cal|X>> as <math|N\<rightarrow\>+\<infty\>>.

    S2) Next is to proof that <math|h<rprime|'>=h\<Rightarrow\>h=p> on
    <math|<with|math-font|cal|X>>. (This proof temporally employs the
    Perron\UFrobenius theorem.)

    S2.1) Indeed, we shall not forget that
    <math|<around*|{|r<rsub|1>,r<rsub|2>,\<ldots\>,r<rsub|N>|}>> are
    generated by a Markov chain obeying Metropolis algorithm. Thus, the
    Metropolis algorithm as a Markov process brings
    <math|r<rsub|i>\<rightarrow\>r<rsub|i+1>> for
    <math|\<forall\>i=1,\<ldots\>,N-1>, s.t.
    <math|<around*|{|r<rsub|1>,r<rsub|2>,\<ldots\>,r<rsub|N-1>|}>\<rightarrow\><around*|{|r<rsub|2>,r<rsub|3>,\<ldots\>,r<rsub|N>|}>>.
    That is to say, <math|h<rprime|'>> can be regarded as being generated by
    Metropolis algorithm from <math|h>. So, <math|h> and <math|h<rprime|'>>
    are related by lemma <reference|lemma: transition>.

    S2.2) Since <math|h> and <math|h<rprime|'>> are related so, as in the
    proof of lemma <reference|lemma: transition> before inserting the
    condition <math|q<around*|(|s\<rightarrow\>r|)>=q<around*|(|r\<rightarrow\>s|)>>,
    ...
    that all components are real and non-negative<\footnote>
      Recall that this corollary requires the Metropolis condition
      <math|q<around*|(|s\<rightarrow\>r|)>=q<around*|(|r\<rightarrow\>s|)>>
      for <math|\<forall\>r,s\<in\><with|math-font|cal|X>>. This is where
      this condition is employed in the proof of Metropolis algorithm.
    </footnote>. So, as a distribution (thus all components have to be real
    and non-negative), <math|\<b-h\>> has no choice but be <math|\<b-p\>>,
    which is what we want to prove, that is
    <math|h<rprime|'>=h\<Rightarrow\>h=p> on <math|<with|math-font|cal|X>>.
  </proof>

  <\remark>
    <label|remark: Burn-in>[Burn-in]
...
<\references>
  <\collection>
    ...
    <associate|remark: Burn-in|<tuple|5|?>>
